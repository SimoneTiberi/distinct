...
unsigned int perm_one_gene_non_zero(arma::vec const& breaks, 
                           arma::mat& cdf_A, 
                           arma::mat& cdf_B,
...
// [[Rcpp::export]]
List perm_test_zero_inflated(unsigned int const& P,                             // number of permutations for all gene-cluster combinations
                             ...
                             arma::sp_mat const& counts)                                // count matrix (rows = genes, cols = cells)
{
  ...
  /* Global variables */
  unsigned int s, tmp2, cell, b, sample, p, n_cells, n_NONZERO;
  ...
  for (unsigned int cl_id = 0; cl_id < n_clusters; ++cl_id) { // loop over clusters
    ...
    // TODO: n_both and n_cells are the same!
    ...
    for (unsigned int gene = 0; gene < n_genes; ++gene) {
      ...
      if (s >= min_non_zero_cells) {
        n_NONZERO = s;
        // TODO: save non-zero elements of all the elements that I need:
        
        arma::uvec sample_ids_one_cl_NONZERO(n_NONZERO); 
        arma::vec counts_one_gene_NONZERO(n_NONZERO);
        
        s = 0;
        for (b=0 ; b<n_both ; b++) {
          if ( counts_one_gene(b) > 0 ) {
            counts_one_gene_NONZERO(s) = counts_one_gene(b);
            sample_ids_one_cl_NONZERO(s) = sample_ids_one_cl(b);
            s += 1;
          }
        }
        ...
        for (sample=0 ; sample<n_samples ; sample++) {
          sample_has_cells(sample) = any(sample_ids_one_cl_NONZERO==sample);
        }
        
        /* CDF grid */ 
        mn = counts_one_gene_NONZERO.min();
        len_out = ( counts_one_gene_NONZERO.max() - mn ) / (N_breaks + 1.0);
        for (b=0 ; b<N_breaks ; b++) {
        ...
        for ( sample=0 ; sample<n_samples ; sample++ ) {
          if ( (sample_has_cells(sample))==1 ) { // check that samples have at least 1 cell
            arma::uvec tmp1 = find( sample_ids_one_cl_NONZERO == sample ); // find the cells associated to the sample `sample`
            tmp2 = tmp1.n_elem; // counts number of cells
            arma::vec counts_one_gene_one_sample(tmp2); // create a vector to store these cells
            for ( b=0 ; b<tmp2 ; b++) { // store cells of sample `sample`
              counts_one_gene_one_sample(b) = counts_one_gene_NONZERO( tmp1(b) );
            }
        ...
        /* permutation martix */ 
        arma::umat PERM(n_NONZERO,P);
        Rcpp::IntegerMatrix PERMUTATIONS(n_NONZERO,P);
        Rcpp::IntegerVector permutation = Rcpp::seq_len(n_NONZERO)-1; // define the vector for each permutation, -1 because Rcpp indexes start from 0 !
        for (p=0 ; p<P ; p++) {
        ...
        tmp_res = 1 + perm_one_gene_non_zero(breaks, 
                                    cdf_A, 
                                    cdf_B,
                                    n_samples,
                                    sample_has_cells,
                                    sample_ids_one_cl_NONZERO,
                                    counts_one_gene_NONZERO,
        ...
        if( p_val <= 0.1 ){ // if p_val < 0.1, use 10 * perm
          if(P_2 > P){
            ...
            arma::umat PERM_2(n_NONZERO,P_2-P);
            Rcpp::IntegerMatrix PERMUTATIONS_2(n_NONZERO,P_2-P);
            for (p=0 ; p<(P_2-P) ; p++) {
              std::random_shuffle(permutation.begin(), permutation.end(), randWrapper);
            ...
            tmp_res += perm_one_gene_non_zero(breaks, 
                                     cdf_A, 
                                     cdf_B,
                                     n_samples,
                                     sample_has_cells,
          ...
          if( p_val <= 0.01 ){ // if p_val < 0.01, use 100 * perm
            if(P_3 > P_2){
              ...
              arma::umat PERM_3(n_NONZERO,P_3-P_2);
              Rcpp::IntegerMatrix PERMUTATIONS_3(n_NONZERO,P_3-P_2);
              for (p=0 ; p<(P_3-P_2) ; p++) {
                std::random_shuffle(permutation.begin(), permutation.end(), randWrapper);
            ...
            if( p_val <= 0.001 ){ // if p_val < 0.001, use 1,000 * perm
              if(P_4 > P_3){
                /* THIRD permutation martix */ 
                arma::umat PERM_4(n_NONZERO,P_4-P_3);
                Rcpp::IntegerMatrix PERMUTATIONS_4(n_NONZERO,P_4-P_3);
                for (p=0 ; p<(P_4-P_3) ; p++) {
                  std::random_shuffle(permutation.begin(), permutation.end(), randWrapper);
